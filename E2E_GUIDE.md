...
Linux and local baselines have separate filenames because Chromium text
rasterization differs between platforms even with identical bundled fonts.
Linux baselines are generated by the manual CI workflow and uploaded as an
artifact for review. They are committed only after visual inspection; CI never
...
The client accepts `e2eRoomCode` and `e2eSeed` fixture values only while
the emulator build flag is enabled. They stabilize generated room codes and
shuffle seeds without bypassing anonymous Auth, room controls,
